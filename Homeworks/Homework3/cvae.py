...
class CVAE(nn.Module):
    def __init__(self, input_size, latent_size, class_size):
        ...
        ######################################################
        #in: input + class -> units -> units -> latent_size
        self.relu = nn.ReLU()
        ...
        ######################################################
        #latent + class???? -> units -> units -> input
    ...
    def recognition_model(self, x, c):
        ...
        ######### TO DO ###########
        ###########################
        x_c = torch.cat((x, c),1) # batch, input + classes
    ...
    def generation_model(self, z, c): # P(x|z, c)
        ...
        ######### TO DO ###########
        ###########################
        z_c = torch.cat((z, c),1) # batch, latent + class
...
def train(epoch, model, train_loader, optimizer, num_classes, use_cuda):
    ...
    for batch_idx, (data, labels) in enumerate(train_loader):
        ...
        '''
        #check data are within 0,1
        if sum(sum((data<0) | (data > 1))) > 0:
            print(data)
            exit(-1)
        '''
