...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, output1, output2, label):
        ...
        ###input is (N,?)??
        #euc_norm = np.linalg.norm(output1 - output2)
        #print(output1.shape,output2.shape, label.shape)
        #euc_norm_1 = (torch.sum((output1 - output2)**2, dim=1))**0.5
        #a=time.time()
        euc_norm = torch.norm(output1 - output2, dim=1)
        #print(euc_norm_1 - euc_norm)
        #print(euc_norm.shape)
        #device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        #zeros = torch.zeros(euc_norm.shape).to(device)
        #b=time.time()
        label = label.squeeze()
        #c=time.time()
        #loss = (1-label)*(euc_norm**2) + label*((torch.max(zeros, self.margin - euc_norm))**2)
        loss = (1-label)*(euc_norm**2) + label*((torch.clamp(self.margin - euc_norm, min=0))**2)
        #d=time.time()
